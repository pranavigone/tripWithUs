...
function sortByPriceInAscending(hotel1, hotel2) {
  return hotel1.price - hotel2.price;
}
function sortByPriceInDescending(hotel1, hotel2) {
...
app.get('/hotels/sort/pricing', (req, res) => {
  ...
  if (pricing === 'low-to-high') {
    hotelsCopy.sort(sortByPriceInAscending);
  } else if (pricing === 'high-to-low') {
    hotelsCopy.sort(sortByPriceInDescending);
  }
  res.json(hotelsCopy);
});
...
app.get('/hotels/sort/rating', (req, res) => {
  ...
  if (rating === 'low-to-high' || rating === 'high-to-low') {
    const order = rating === 'high-to-low' ? -1 : 1;
    hotelsCopy.sort(
      (hotel1, hotel2) => (hotel1.rating - hotel2.rating) * order
    );
  } else {
...
app.get('/hotels/sort/reviews', (req, res) => {
  ...
  if (reviews === 'least-to-most' || reviews === 'most-to-least') {
    const order = reviews === 'most-to-least' ? -1 : 1;
    hotelsCopy.sort(
      (hotel1, hotel2) => (hotel1.reviews - hotel2.reviews) * order
    );
  } else {
    return res.status(400).json({
      error: "Invalid pricing value. Use 'least-to-most' or 'most-to-least'.",
    });
  }
  res.json(hotelsCopy);
});
...
app.get('/hotels/filter/amenity', (req, res) => {
  ...
  let filterByAmenity = filterHotelsByAmenity(hotels, amenity);
  res.json(filterByAmenity);
});
...
app.get('/hotels/filter/country', (req, res) => {
  ...
  let filterByCountry = filterHotelsByCountry(hotels, country);
...
app.get('/hotels/filter/category', (req, res) => {
  ...
  let filterByCategory = filterHotelsByCategory(hotels, category);
